...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate the pretrained model on real data without fine-tuning.")
...
def main() -> None:
    ...
    run_dir = create_run_dir(output_root, "04_eval_baseline", config["experiment"]["name"])
    ...
    checkpoint_path = resolve_project_path(
        PROJECT_ROOT,
        eval_config.get("baseline_checkpoint") or init_config.get("pretrained_checkpoint"),
    )
    ...
    save_trajectory_plot(
        ...
        fps=window_spec.fps,
        output_path=run_dir / "baseline_trajectory_plot.png",
        title=f"Baseline Real-Data Prediction | sample={test_sample_ids[chosen_index]}",
    ...
    save_error_histogram(
        ...
        output_path=run_dir / "baseline_error_histogram.png",
        title="Baseline Real-Data MAE Distribution",
        bins=int(plot_config["histogram_bins"]),
    )

    print(f"Baseline metrics saved to: {run_dir / 'baseline_metrics.json'}")
